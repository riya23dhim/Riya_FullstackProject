...
let NotificationsService = class NotificationsService {
    ...
    async findAll(userId, page = 1, limit = 10) {
        ...
        const notifications = await this.notificationModel
            ...
            .limit(limit)
            .exec();
        const total = await this.notificationModel.countDocuments({ user: userId });
        return { data: notifications, total, page, limit };
    }
    async markAsRead(id, userId) {
        const notification = await this.notificationModel.findOneAndUpdate({ _id: id, user: userId }, { isRead: true }, { returnDocument: 'after' }).exec();
        if (!notification)
